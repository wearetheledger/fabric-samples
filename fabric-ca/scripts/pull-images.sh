...
dockerCaPull() {
    ...
    for IMAGES in peer orderer tools ; do
      echo "==> FABRIC IMAGE: $IMAGES"
      echo
      docker pull hyperledger/fabric-ca-$IMAGES:$CA_TAG
      docker tag hyperledger/fabric-ca-$IMAGES:$CA_TAG hyperledger/fabric-ca-$IMAGES
    done
